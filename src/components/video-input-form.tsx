...
type TVideoInputFormProps = {
  onVideoUploaded: (id: string) => void;
}

export function VideoInputForm({ onVideoUploaded }: TVideoInputFormProps) {
  ...
  const handleUploadVideo = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await api.post<{video: {id: string}}>('/videos', data);

    const videoId = response.data.video.id;

    setStatus('generating');

    await api.post(`http://localhost:3333/videos/${videoId}/transcription`, {
      prompt,
    });
